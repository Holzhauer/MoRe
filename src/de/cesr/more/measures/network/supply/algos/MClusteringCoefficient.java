...
public class MClusteringCoefficient {
	 ...
	 * 
	 * Vertices with less than two neighbours contribute 0.0. See, for instance, Vega-Redondo2007, p. 33
	 * 
	 * @param <V>
	 * @param <E>
	 * @param graph
	 * @return the clustering coefficient Created by Sascha Holzhauer on 12.04.2010
	 */
	public static <V, E> double getClusteringCoefficientPerNode(Graph<V, E> graph) {
		logger.info("Calculate Clustering Coefficient per Node for a graph containing " + graph.getVertexCount()
				+ " nodes.");
		double sum = 0.0;
		double vsum;
	 ...
	 * Calculates the clustering coefficient (also known as transitivity) for the given graph.
	 * 
	 * Vertices with less than two neighbours contribute not at all. See, for instance, Vega-Redondo2007, p. 33
	 * 
	 * @param <V>
	 * @param <E>
	 * @param graph
	...
	public static <V, E> double getClusteringCoefficientPerNodeWithNeighbors(Graph<V, E> graph) {
		logger.info("Calculate Clustering Coefficient per Node with Neighbors for a graph containing "
				+ graph.getVertexCount() + " nodes.");
		double sum = 0.0;
		double vsum;
		int lonelyCounter = 0;
		for (V v : graph.getVertices()) {
	 ...
	 * Calculates the clustering coefficient (also known as transitivity) for the given graph.
	 * 
	 * TODO check why this leads to different results than R implementation! > the formula in Vega-Rodondo2007 seems to
	 * be wrong!
	 * 
	 * The ration of all triples and all triangles - gives more weight to high-degree nodes. Straight implementation of
	 * e.g. Vega-Rodondo2007 p. 34 (Vertices with less than two neighbours contribute not at all.)
	 * 
	 * @param <V>
	 * @param <E>
	 * @param graph
	...
	public static <V, E> double getClusteringCoefficientOverallRatio(Graph<V, E> graph) {
		logger.info("Calculate Clustering Coefficient overall ration for a graph containing " + graph.getVertexCount()
				+ " nodes.");
		double vsum = 0.0;
		int counter = 0;
	 ...
	 * Calculates the clustering coefficient (also known as transitivity) for the given graph.
	 * 
	 * The algorithm conforms the one implemented in igraph (R library). It counts the overall number of triples and
	 * calculated the ratio of triples and triangles.
	 * 
	 * @param <V>
	 * @param <E>
	 * @param graph
	...
	public static <V, E> double getClusteringCoefficientRlike(final Graph<V, E> graph) {
		logger
				.info("Calculate Clustering Coefficient (R) for a graph containing " + graph.getVertexCount()
						+ " nodes.");
		int triangels = 0;
		int triples = 0;
	 ...
	 * 
	 * 
	 * The ration of all triples and all triangles - gives more weight to high-degree nodes. Straight forward according
	 * to Goyal2009 p. 19 (Vertices with less than two neighbours contribute not at all.) Corresponds to R
	 * implementation.
	 * 
	 * @param <V>
	 * @param <E>
	...
	public static <V, E> double getClusteringCoefficientWeightedRatio(Graph<V, E> graph) {
		logger.info("Calculate Clustering Coefficient weighted ration for a graph containing " + graph.getVertexCount()
				+ " nodes.");
	...
	/**
	 * Does not consider the ego itself... As described e.g. in Watts1998, corresponds to Vega-Rodondo2007 (less
	 * efficient) (Vertices with less than two neighbours contribute 0.0.)
	 * 
	 * @param <V>
	 * @param <E>
	 * @param graph
	 * @return Created by Sascha Holzhauer on 06.05.2010
	 */
	public static <V, E> double getClusteringCoefficientWS(Graph<V, E> graph) {
		logger.info("Calculate Clustering Coefficient after Watts/Strogatz for a graph containing "
				+ graph.getVertexCount() + " nodes.");
		double sum = 0.0;
		double vsum = 0.0;
		for (V v : graph.getVertices()) {
			for (V n : graph.getNeighbors(v)) {
				for (V m : graph.getNeighbors(v)) {
					if (true) {
						if (graph.isNeighbor(m, n))
							vsum++;
					}
				}
			}
			sum += vsum / ((graph.getNeighborCount(v)) * (graph.getNeighborCount(v) - 1.0));
			vsum = 0;
		}
		return sum / graph.getVertexCount();
	}
