...
public class MGeoTorusDistanceQuery<AgentType> extends
	...
	protected void init(Geography<AgentType> geography, double distance,
			Geometry location) {
		...
		try {
			...
			if (convert) {
				...
				p = (Point) JTS.transform(p, cFactory.createOperation(crs, utm)
						.getMathTransform());
				// <- LOGGING
	...
	protected Predicate<AgentType> createPredicate() {
		if (sourceObject != null) {
			return new IntersectsPredicate<AgentType>();
		} else {
			return new NoSourceIntersectsPredicate<AgentType>();
		}
	}

	private class NoSourceIntersectsPredicate<AgentT> implements
			Predicate<AgentT> {

		@Override
		public boolean evaluate(AgentT o) {
			logger.debug("Evaluate (NoSourceIntersectsPredicate): " + o);
	...
	private class IntersectsPredicate<AgentT> implements Predicate<AgentT> {
		...
		/**
		 * TODO does not work with MTorusCoordinate when coordinates need be
		 * transformed (when CRS is not metric)! > possible solution -> exchange
		...
		public boolean evaluate(AgentT o) {
			...
			logger.debug("Evaluate (IntersectsPredicate): " + o);
			
			Geometry other = geography.getGeometry(o);
			if (!o.equals(sourceObject) && other instanceof Point) {

				if (convert) {
					CoordinateReferenceSystem utm = UTMFinder.getUTMFor(other,
							geography.getCRS());
					try {
						other = JTS.transform(
								other,
								cFactory.createOperation(geography.getCRS(),
										utm).getMathTransform());
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
				logger.debug(o + "> " + other.distance(geom));
				return other != null && other.distance(geom) <= distance;
			}
			return false;
		}
